<?php
    ...
    try {
        switch ($cmd) {
            ...
            case "most_recent_post":
                $response = mostRecentPost($conn, $response);
                break;
            case "most_recent_post_admin":
                $response = mostRecentPostAdmin($conn, $response);
                break;
            case "check_permissions":
                $response = checkPermissions($conn, $response);
            break;
            default:
    ...
    function listAll($conn, $response) {
        $stmt = "SELECT CreatorID, Email, Snippet_Data.LanguageID, LanguageName, SnippetID, Description, Code, Time FROM Snippet_Data INNER JOIN User_Data ON Snippet_Data.CreatorID = User_Data.UserID INNER JOIN Language_Data ON Snippet_Data.LanguageID = Language_Data.LanguageID;";
...
// verify inputs
    ...
    function verifyResetPasswordInputs($conn, $email, $resetPass, $resetPassConfirm, $answer1, $answer2) {
        ...
        if ($findUserResponse["user"]->getEmail() != null) {
            ...
            throw new Exception("A user with that username wasn't found.");
        }
...
// prepared statement functions
    ...
    function updateSnippet($conn, $response) {
        ...
        if($user->getID() == findSnippetCreatorID($conn, $snippetID)["creatorID"] || checkPermissions($conn)["permission"] == "ADMIN"){
    ...
    function mostRecentPost($conn, $response){
        $user = findUser($conn, $_SESSION["email"])["user"];
        $stmt = $conn->prepare("SELECT t1.SnippetID FROM Snippet_Data t1 WHERE time = (SELECT MAX(time) FROM Snippet_Data t2 WHERE t1.CreatorID = ?); ");
        $stmt->bind_param("i", $user->getID());
        $stmt->execute();
        $stmt->bind_result($snippetID);
        $stmt->fetch();
        $stmt->close();
        $response["status"] = "OK";
        $response["ID"] = $snippetID;
    ...
    function mostRecentPostAdmin($conn, $response){
        $user = findUser($conn, $_SESSION["email"])["user"];
        $snippetID = $_POST["snippetID"];
        $creatorID = findSnippetCreatorID($conn, $snippetID)["creatorID"];
        $stmt = $conn->prepare("SELECT t1.SnippetID FROM Snippet_Data t1 WHERE time = (SELECT MAX(time) FROM Snippet_Data t2 WHERE t1.CreatorID = ?); ");
    ...
    function checkPermissions($conn){
        $user = findUser($conn, $_SESSION["email"])["user"];
        $stmt = $conn->prepare("SELECT Permission FROM User_Data WHERE UserID = ?");
        $stmt->bind_param("i", $user->getID());
        $stmt->execute();
