...
function updateActiveSnippet() {
    ...
    document.getElementById('snippet-date').innerHTML = snippet.time;
    if (snippet.creator == currentUser || currentPermissions == "ADMIN") {
...
function getUserPermission(){
    let url = SnippetsUrl + '?cmd=check_permissions';
    $.get(url, function(response) {
        currentPermissions = response.permission;
    });
}
...
function logoutUser() {
    ...
    .fail(function(){
        userAlert('danger', 'Error has occured logging out.');
    })
    .always(function(){
...
function createSnippet(e) {
        ...
        if (formValid) {
            ...
            .done(function(data) {
                if (data.status === "OK") {
                    userAlert('success', 'Snippet posted successfully.');
                    getSnippets();
                    setRecentSnippet();
                } else {
                    userAlert('danger',  data.errmsg);
            ...
            .fail(function(data) {
                userAlert('danger', 'Failed to post snippet.');
            })
            .always(function(data) {
                snippetName.val('');
                language.val('');
...
//gets the users most recent snippet and sets it to display
function setRecentSnippet(){
    let url = SnippetsUrl + '?cmd=most_recent_post';
    $.get(url)
    .done(function(data) {
        if (data.status === "OK") {
            snippetsTable.rows().every( function () {
...
function setRecentSnippetAdmin(){
    let url = SnippetsUrl + '?cmd=most_recent_post_admin';
    let snippet = model.getSelectedSnippet();
    $.post(url, {snippetID: snippet.id})
    .done(function(data) {
        if (data.status === "OK") {
            snippetsTable.rows().every( function () {
                if(this.data()[0] == data.ID){
...
function deleteSnippet(e) {
    ...
    .fail(function(data) {
        userAlert('danger', 'Failed to delete snippet.');
    })
    }
}
